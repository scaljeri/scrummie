Template.manipTask.created = function () {
    this.dep = new Tracker.Dependency();
};
Template.manipTask.rendered = function () {
    App.eventHub.on('manipTask.show', show.bind(this));
};

Template.manipTask.helpers({
    task: function () {
        Template.instance().dep.depend();
        return Template.instance().selectedTask;
    },

    jiraEnabled: function () {
        Template.instance().dep.depend();
        return jiraEnabled.call(Template.instance());
    },
    colors: function () {
...
function show(task, callback) {
    ...
    if (!task && jiraEnabled.call(this) === true) {
    ...
    App.outsideClick.register('[manip-task]', hide);
}
...
function jiraEnabled() {
    ...
    if (settings && settings.connections.jira && !Session.get('jiraDisabled')) {
        retVal = !this.selectedTask && settings.connections.jira.checked;
    }
